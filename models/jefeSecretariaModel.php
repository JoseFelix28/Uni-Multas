...
class JefeSecretariaModel extends Model implements IModel{
    ...
    public function save(){
        try{
            $query = $this->prepare("UPDATE `users` SET (role)='JefeSecretaria' WHERE users.id=:id");
            $query->execute([
                'role' => $this->role
            ]);
    ...
    public function delete($id){
        try{
            $query = $this->db->connect()->prepare("UPDATE `users` SET (role)='user' WHERE users.id=:id");
            $query->execute([ 'id' => $id]);
            return true;
        }catch(PDOException $e){
            echo $e;
