...
class SessionController extends Controller{
    ...
    function getUserSessionData(){
        ...
        $this->user = new UsersFatherModel();
        $this->user->get($id);
    ...
    private function redirectDefaultSiteByRole($role){
        ...
        for($i = 0; $i < sizeof($this->sites); $i++){
            if($this->sites[$i]['role'] === $role){
                $url = '/Uni-Multas/'.$this->sites[$i]['site'];
            break;
            }
        }
        header('location: '.$url);
    ...
    function authorizeAccess($role){
        ...
        switch($role){
            case 'user':
                ...
                error_log("sessionController::getCurrentPage: Inicia como Usuario");
            break;
            case 'funcionario':
                $this->redirect($this->defaultSites['funcionario']);
                error_log("sessionController::getCurrentPage: Inicia como Funcionario");
            break;
            case 'jefeSecretaria':
                $this->redirect($this->defaultSites['jefeSecretaria']);
                error_log("sessionController::getCurrentPage: Inicia como Jefe de Secretaria");
            break;
            case 'secretaria':
                $this->redirect($this->defaultSites['secretaria']);
                error_log("sessionController::getCurrentPage: Inicia como Secretaria");
            break;
            case 'admin':
                $this->redirect($this->defaultSites['admin']);
                error_log("sessionController::getCurrentPage: Inicia como Admin");
            break;
            default:
        }
    }
